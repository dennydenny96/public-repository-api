...
{
    public class ComboJobTitle : BaseService
    {
        public ComboJobTitle(HttpContext httpContext) : base(httpContext)
        {
        }
        ...
        public override string GetResponse()
        {
            ...
            try
            {
                ...
                {
                    var tuple = this.ParsingJSONStringInput<Model.ComboJobTitle>();
                    if (tuple.Item1)
                    {
                        var data = tuple.Item3;

                        retVal = this.ProcessResult(Helper.ComboHelper.ComboJobTitle(data));
                    }
                    else
                    {
                        retVal = tuple.Item2;
