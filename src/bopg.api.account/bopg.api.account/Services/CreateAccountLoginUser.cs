...
{
    public class CreateAccountLoginUser : BaseService
    {
        public CreateAccountLoginUser(HttpContext httpContext) : base(httpContext)
        {
        }
        ...
        public override string GetResponse()
        {
            ...
            try
            {
                ...
                {
                    var tuple = this.ParsingJSONStringInput<Model.UserLoginAdd>();
                    if (tuple.Item1)
                    {
                        var data = tuple.Item3;

                        retVal = this.ProcessResult(Helper.UserLoginHelper.CreateAccountLoginUser(data));
                    }
                    else
                    {
                        retVal = tuple.Item2;
