...
{
    public class GetUserList : BaseService
    {
        public GetUserList(HttpContext httpContext) : base(httpContext)
        {
        }
        ...
        public override string GetResponse()
        {
            ...
            try
            {
                ...
                {
                    var tuple = this.ParsingJSONStringInput<Model.GetUserList>();
                    if (tuple.Item1)
                    {
                        var data = tuple.Item3;

                        retVal = this.ProcessResult(Helper.GetUserListHelper.GetUserList(data));
                    }
                    else
                    {
                        retVal = tuple.Item2;
