...
namespace bopg.api.account.Helper
{
    ...
    {
        public static BaseHelper MenuList(Model.MenuList data)
        {
            var retVal = new BaseHelper();
            var objJSONPage = new Output.MenuList();
            try
            {
                var entityPage = Menu.MenuList(data, objJSONPage);
            ...
            {
                retVal.SerializeObject<Output.MenuList>(objJSONPage);
            }
